...
def predict(X_img_path, knn_clf=None, model_path=None, distance_threshold=0.50):
    ...
    return [(pred, loc) if rec else ("unknown/fake", loc) for pred, loc, rec in zip(knn_clf.predict(faces_encodings), X_face_locations, are_matches)]


def show_prediction_labels_on_image(img_path, predictions, img):
    """
    ...
    img = cv2.imread("../FakeImageDetection/test/"+str(img))
...
if __name__ == "__main__":
    ...
    '''for image_file in os.listdir("../FakeImage/test"):
        full_file_path = os.path.join("../FakeImage/test", image_file)
        ...
        show_prediction_labels_on_image(os.path.join("../FakeImage/test", image_file), predictions)'''
